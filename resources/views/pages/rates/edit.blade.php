            ...
            <div id="view-validations">
              <form class="formValidate" action="{{ route('rates.update', $rate->id) }}" id="formValidate" method="post">
                ...
                <div class="row">
                  ...
                  <div class="input-field col s12">
                    <label for="max">
                      @if(canEditLang() && checkRequestOnEdit())<editor_block data-name='Maximum deposit *' contenteditable="true">{{ __('Maximum deposit *') }}</editor_block>@else {{ __('Maximum deposit *') }} @endif</label>
                    <input id="max" type="number" name="max" data-error=".errorTxt3" value="{{ number_format($rate->max, 2, '.', '') }}">
                    <small class="errorTxt3"></small>
                  </div>
                  <div class="input-field col s12">
                    <label for="daily">
                      @if(canEditLang() && checkRequestOnEdit())<editor_block data-name='Daily percentage *' contenteditable="true">{{ __('Daily percentage *') }}</editor_block>@else {{ __('Daily percentage *') }} @endif</label>
                    <input id="daily" type="text" name="daily" data-error=".errorTxt55" value="{{ number_format($rate->daily, 2, '.', '') }}">
                    <small class="errorTxt55"></small>
                  </div>
                  <div class="input-field col s12">
                    <label for="duration">
                      @if(canEditLang() && checkRequestOnEdit())<editor_block data-name='Deposit validity *' contenteditable="true">{{ __('Deposit validity *') }}</editor_block>@else {{ __('Deposit validity *') }} @endif</label>
                    <input id="duration" type="number" name="duration" data-error=".errorTxt4" value="{{ $rate->duration }}">
                    <small class="errorTxt4"></small>
                  </div>
                  <div class="input-field col s12">
                    <label for="tes">
                      @if(canEditLang() && checkRequestOnEdit())<editor_block data-name='% Return of the deposit at the end of the term (% or leave blank)' contenteditable="true">{{ __('% Return of the deposit at the end of the term (% or leave blank)') }}</editor_block>@else {{ __('% Return of the deposit at the end of the term (% or leave blank)') }} @endif</label>
                    <input id="tes" type="text" name="overall" data-error=".errorTxt4" value="{{ $rate->overall ?? '' }}">
                    <small class="errorTxt4"></small>
                  </div>
                  <div class="col s12">
                    <label for="reinvest">
                      @if(canEditLang() && checkRequestOnEdit())<editor_block data-name='The ability to reinvest' contenteditable="true">{{ __('The ability to reinvest') }}</editor_block>@else {{ __('The ability to reinvest') }} @endif</label>
                    <p>
                      <label>
                        <input type="hidden" name="reinvest" value="0">
                        <input type="checkbox" name="reinvest" id="reinvest" value="1" {{ $rate->reinvest ? 'checked' : ''}}/>
                        <span>
@if(canEditLang() && checkRequestOnEdit())<editor_block data-name='Yes/No' contenteditable="true">{{ __('Yes/No') }}</editor_block>@else {{ __('Yes/No') }} @endif</span>
                      </label>
                    </p>
                  ...
                  <div class="col s12">
                    <label for="autoclose">
                      @if(canEditLang() && checkRequestOnEdit())<editor_block data-name='Auto closing of the deposit' contenteditable="true">{{ __('Auto closing of the deposit') }}</editor_block>@else {{ __('Auto closing of the deposit') }} @endif</label>
                    <p>
                      <label>
                  ...
                  <div class="col s12">
                    <label for="active">
                      @if(canEditLang() && checkRequestOnEdit())<editor_block data-name='Active' contenteditable="true">{{ __('Active') }}</editor_block>@else {{ __('Active') }} @endif</label>
                    <p>
                      <label>
                  ...
                  <div class="input-field col s12">
                    <button class="btn waves-effect waves-light right submit" type="submit" name="action" @if(canEditLang() && checkRequestOnEdit()) onclick="event.preventDefault()" @endif>
