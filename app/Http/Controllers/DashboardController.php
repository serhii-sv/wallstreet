    ...
    public function index()
    {
        ...
        foreach ($weeks_period as $key => $week) {
            $transactions = cache()->remember(
                'dshb.last_transactions'.$week['start'],
                60,
                function () use ($week) {
                    return Transaction::where('approved', 1)->whereBetween(
                        'created_at',
                        [
                            $week['start'],
                            $week['end'],
        ...
        foreach ($payment_system as $item) {
            $item->transaction_sum   = cache()->remember(
                'dshb.payment_transactions_sum'.$item->id,
                60,
                function () use ($item) {
                    return $item->transactions_enter()->sum('main_currency_amount');
                }
            );
            $item->transaction_minus = cache()->remember(
                'dshb.payment_transaction_minus'.$item->id,
                60,
                function () use ($item) {
        ...
        foreach ($month_period as $key => $month) {
            $transactions = cache()->remember(
                'dshb.last_transactions'.$month['start'],
                60,
                function () use ($month) {
                    return Transaction::where('approved', 1)->whereBetween(
                        'created_at',
        ...
        $countries_stat->map(
            function ($country) use ($id_enter) {
                $country->count = cache()->remember(
                    'dshb.countries_stat_count_'.$country->name,
                    60,
        ...
        $countries_stat->map(
            function ($country) use ($id_enter) {
                $country->invested = 0;
                User::where('country', $country->name)->get()->map(
                    function ($user) use ($country, $id_enter) {
                        $country->invested += cache()->remember(
                            'dshb.countries_stat_invested_'.$user->id,
                            60,
                            function () use ($country, $id_enter, $user) {
        ...
        $cities_stat->map(
            function ($city) use ($id_enter) {
                $city->count = cache()->remember(
                    'dshb.city_stat_count_'.$city->name,
                    60,
                    function () use ($city) {
        ...
        $enter_transactions_for_24h_sum = Cache::remember(
            'dshb.transactions.enter.for_24h',
            60,
            function () {
                return Transaction::where('created_at', '>=', Carbon::now()->subDay()->format('Y-m-d H:i:s'))->where('approved', '=', 1)->whereNotNull('payment_system_id')->whereHas(
                    'type',
                    function ($query) {
        ...
        $enter_transactions_for_today_sum = Cache::remember(
            'dshb.transactions.enter.for_today',
            60,
            function () {
                return Transaction::where('created_at', '>=', Carbon::now()->startOfDay()->format('Y-m-d H:i:s'))->where('approved', '=', 1)->whereNotNull('payment_system_id')->whereHas(
                    'type',
                    function ($query) {
                        $query->where('name', 'enter');
                    }
        ...
        $withdraw_transactions_for_24h_sum = Cache::remember(
            'dshb.transactions.withdraw.for_24h',
            60,
            function () {
                return Transaction::where('created_at', '>=', Carbon::now()->subDay()->format('Y-m-d H:i:s'))->where('approved', '=', 1)->whereNotNull('payment_system_id')->whereHas(
                    'type',
        ...
        $withdraw_transactions_for_today_sum = Cache::remember(
            'dshb.transactions.withdraw.for_today',
            60,
            function () {
                return Transaction::where('created_at', '>=', Carbon::now()->startOfDay()->format('Y-m-d H:i:s'))->where('approved', '=', 1)->whereNotNull('payment_system_id')->whereHas(
                    'type',
        ...
        return view(
            ...
            [
                ...
                'users'                              => [
                    'online' => $this->users->where(
                        'last_activity_at',
                        '>',
                        Carbon::now()->subSeconds(config('chats.max_idle_sec_to_be_online'))->format('Y-m-d H:i:s')
                    )->get(),
                    'total'  => $this->users->all()->count(),
                    'today'  => $this->users->where(
                        'created_at',
                        '>',
                        Carbon::now()->subDay()->format('Y-m-d H:i:s')
                    )->get()->count(),
                ],
                'deposit_total_sum'                  => Cache::remember(
                    'dshb.transactions.enter.total',
                    60,
                    function () {
                        return Transaction::where('approved', '=', 1)->whereNotNull('payment_system_id')->whereHas(
                            'type',
                            function ($query) {
                                $query->where('name', 'enter');
                            }
                ...
                'deposit_total_withdraw'             => Cache::remember(
                    'dshb.transactions.withdraw.total',
                    60,
                    function () {
                        return Transaction::where('approved', '=', 1)->whereNotNull('payment_system_id')->whereHas(
                            'type',
